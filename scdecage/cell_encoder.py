...
class PretrainedCellEncoder(nn.Module):
    """Encode ranked gene-expression tokens into a cell representation."""
    ...
    def __init__(self, vocab_size: int, padding_idx: int = 0, embed_dim: int = 128,
        ...
        self.total_counts_embed = nn.Linear(1, embed_dim)
    ...
    def forward(self, gene_ids: torch.Tensor, expression_values: torch.Tensor,
                total_counts: torch.Tensor | None = None) -> torch.Tensor:
        valid = gene_ids.ne(self.padding_idx)
        x = self.gene_embed(gene_ids) + self.value_embed(expression_values.unsqueeze(-1))
        if total_counts is None:
            total_counts = torch.zeros(gene_ids.shape[0], 1, device=gene_ids.device,
                                       dtype=expression_values.dtype)
        cls = self.cls_token + self.total_counts_embed(total_counts.unsqueeze(-1))
        x = torch.cat([cls.expand(gene_ids.shape[0], -1, -1), x], dim=1)
        ...
        return torch.cat([x[:, 0], gene_representation], dim=-1)


def load_pretrained_cell_encoder(checkpoint_path: str | Path, vocab_size: int,
                                 padding_idx: int = 0) -> PretrainedCellEncoder:
    checkpoint = torch.load(checkpoint_path, map_location="cpu", weights_only=False)
    state = checkpoint.get("model", checkpoint)
    encoder = PretrainedCellEncoder(vocab_size=vocab_size, padding_idx=padding_idx)
